...
public void onGuildMessageReceived (GuildMessageReceivedEvent event) {
    	String imgurCmd = "1";
    	if (Main.config.getProperty("imgurCmd" + event.getGuild().getId()) != null) {
    	imgurCmd = Main.config.getProperty("imgurCmd" + event.getGuild().getId());
    	}
		try {
    	String prefix = Main.config.getProperty(event.getGuild().getId());
    	if (prefix == null)
    		prefix = "!";
        if (event.getMessage().getContentRaw().toLowerCase().startsWith(prefix + "imgursearch") && !event.getAuthor().isBot() && imgurCmd.equals("1")) {//&& event.getGuild().getId().equals("468440854886088714")) { 
